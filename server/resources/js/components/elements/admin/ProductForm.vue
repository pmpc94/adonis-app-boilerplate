<template>
  <div>
    <v-form>
      ...
      <h2>Category: </h2>
      <v-select outline name="category" v-validate="'required'" :items="categories" :value="category" @input="$emit('onInputCategory', $event)" placeholder="ex: terrestrial"></v-select>
      ...
      <h2>Price: </h2>
      <v-text-field outline name="price" v-validate="'required'" type="number" min="0" :value="price" @input="$emit('onInputPrice', $event)" @keydown="$emit('preventUndesiredChars', $event)" placeholder="ex: 33" class="headline mb-0"></v-text-field>
